      ...
      <div className="mx-auto max-w-6xl relative z-10">
        ...
        <div className="space-y-10 sm:space-y-12">
          {DISPLAY_CATEGORIES.map((category) => {
            const dishesInCategory = PREDEFINED_DISHES.filter((d) => d.category === category);
            return (
              <section key={category} aria-label={`Pratos ${category}`} className="space-y-5">
                <h3 className="flex items-center justify-center gap-2 font-display text-2xl sm:text-3xl text-corn uppercase tracking-wide">
                        ...
                        {photoUrl ? (
                          ...
                          >
                            ...
                            <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 flex flex-col items-center text-center gap-1.5">
                              <h4 className="font-display text-base sm:text-lg text-white uppercase tracking-wide leading-tight drop-shadow">
                                {dish.name}
                              </h4>
                              {claimed ? (
                          ...
                          >
                            ...
                            <h4 className={`font-display text-lg sm:text-xl uppercase tracking-wide leading-tight ${claimed ? "text-foreground/50" : ""}`}>
                              {dish.name}
                            </h4>
                            {claimed ? (
